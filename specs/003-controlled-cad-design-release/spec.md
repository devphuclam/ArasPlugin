...
- **Authority evidence boundary**: The checked-in submit method accepts only the CAD identifier and comment. The product owner confirmed that Aras Assign/workflow assignment selects the reviewer; the engineer does not select a reviewer in the client. The client MUST NOT invent an Aras property, encode a reviewer identity into a comment, or treat checkout-lock ownership as review ownership. Authority-assigned reviewer consumption and owner-authorized withdrawal remain runtime-disabled until the corresponding authority contracts are verified in evidence. These limitations do not satisfy the full FR-005/FR-006 behavior and are tracked as open tasks rather than claimed as complete.
...
After checking in the completed design, the engineer submits the Part-linked CAD revision for review. The PDM authority assigns and notifies the reviewer through its workflow. The reviewer opens the design, inspects it, and either approves it (moving it to Released) or requests rework (returning it to the engineer for further changes).
...
1. **Given** a checked-in Part-linked CAD revision at the `Thiet ke chi tiet` state, **When** the owning engineer submits it for review with a description of changes, **Then** the revision transitions to `In Review`, Aras Assign/workflow records an eligible reviewer and sends the notification, and the engineer can no longer edit it without withdrawing the submission.
...
3. **Given** a CAD revision at `In Review`, **When** the authority-assigned reviewer chooses Request Rework with a written explanation, **Then** the CAD revision and linked Part revision return to `Thiet ke chi tiet`, neither receives a new engineering revision/version, the engineer is notified with the rework reason, and the pair becomes editable again after a new checkout.

4. **Given** a CAD revision submitted for review, **When** the owning engineer withdraws the submission before the reviewer acts, **Then** the revision returns to `Thiet ke chi tiet` without any review record other than the withdrawal event. **Note**: Withdrawal depends on an authority operation and a verified submission-owner field/permission (GATE-W / GATE-W-owner). The current Aras environment exposes no Withdraw method or lifecycle transition and `LockOwnerName` is checkout ownership only, not review ownership, so the Withdraw UI and FR-006 runtime behavior remain disabled and are not claimed as complete.
...
- **FR-003**: The system MUST allow the checkout owner to check in the modified CAD file with a required written reason. Check-in MUST validate the local file (exists, readable, SHA-256 calculated before upload) and upload the validated file. The client MUST pass the written reason to the authority as the check-in comment. The remaining FR-003 expectations — recording a completed ChangeSet with the persisted reason, generating an audit event, and releasing the checkout lock — are **GATE-B-checkin-conditioned**: they depend on the deployed authority providing atomic update+unlock, ChangeSet creation, comment/reason persistence, and an audit event. The client MUST NOT simulate or assume these authority-side behaviors. **If GATE-B-checkin evidence is not satisfied, full FR-003 (and the related FR-018 "no partial state" guarantee) compliance cannot be claimed**; the check-in path is gated accordingly and the limitation is documented in `docs/evidence/gate-b-checkin-commit-atomicity.md`.
...
- **FR-005**: The system MUST allow the engineer who owns the checked-in CAD revision to submit it for review with a change description. Submission MUST transition the revision to `In Review`; Aras Assign/workflow MUST assign and notify an eligible reviewer. The client MUST consume the authoritative active assignment and MUST NOT ask the engineer to select an arbitrary reviewer, simulate assignment locally, or encode reviewer identity in the comment. **GATE-RS blocks runtime submission/reviewer-decision enablement until the authority's assignment record and active-assignment read contract are verified.**
- **FR-006**: The system MUST allow the engineer to withdraw a submitted revision before the reviewer acts. Withdrawal MUST return the revision to `Thiet ke chi tiet` without recording a review decision. **GATE-W-owner blocks runtime withdrawal until the authority exposes a verified submission-owner field/permission.** Checkout-lock ownership alone is not sufficient evidence of review ownership.
...
- **FR-008**: The system MUST allow the authority-assigned Reviewer to request rework on a submitted CAD revision with a written explanation. MVP Request Rework semantics are **coordinated and state-only**: the CAD Revision and linked Part Revision return to `Thiet ke chi tiet`; neither creates a new engineering revision/version, and the engineer is notified. The deployed `Sync_Part_From_CAD` path is accepted for this state-only coordination because the product owner confirmed that duplicate synchronization is a no-op and Part returns without a new version. **GATE-RW remains an evidence gate for the deployed result and audit behavior; it no longer blocks on an unresolved business-policy decision.**
...
- **Reviewer assignment**: The PDM authority (Aras Assign/workflow) is the authority-assigned reviewer. The client MUST consume the authoritative active assignment and MUST NOT ask the engineer to select a reviewer, simulate assignment locally, or encode a reviewer identity in the comment. This behavior is not yet established by a verified active-assignment read contract and remains gated by GATE-RS; submission and reviewer-decision runtime actions stay disabled until that contract is verified. No placeholder identity or local-only assignment is permitted.
